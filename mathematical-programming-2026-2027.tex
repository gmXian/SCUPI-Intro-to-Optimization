...
\begin{frame}{Global And Local Solutions}
  ...
  \begin{itemize}
    ...
    \item Global minimum:
      \begin{equation*}
        f(\bar{\vect{x}})\le f(\vect{x}),\quad\forall\vect{x}\in\Omega.
      \end{equation*}
    \item Strict global minimum:
      \begin{equation*}
        f(\bar{\vect{x}})<f(\vect{x}),\quad\forall\vect{x}\in\Omega\setminus\{\bar{\vect{x}}\}.
      \end{equation*}
  \end{itemize}
\end{frame}
...
\begin{frame}{Model Stability}
  \footnotesize
  \begin{itemize}
    \item Model Stable: small changes in the data $\rightarrow$ small changes in $x_1^*,\ldots,x_n^*$ and $f(x_1^*,\ldots,x_n^*)$
    \item Model Unstable: small changes in the data $\rightarrow$ LARGE changes in $x_1^*,\ldots,x_n^*$ and $f(x_1^*,\ldots,x_n^*)$
...
\begin{frame}{A Review of Linear Algebra: Vector}
  \begin{itemize}
    ...
    \item Row matrix:
      ...
      \end{equation*}
    \item Scalars: real/complex numbers
    \item Vector space ($\R^{m\times1}$): Euclidean $m$-space ($E^m$)
  \end{itemize}
\end{frame}
...
\begin{frame}{A Review of Linear Algebra: Subspace}
  \footnotesize
  \begin{itemize}
    \item Subspace $S$ of $\R^{n\times1}$: Any nonempty set of vectors, such that $\vect{x},\vect{y}\in S$, $\alpha,\beta\in\R$, and $\alpha\vect{x}+\beta\vect{y}\in S$
...
\begin{frame}{A Review of Linear Algebra: Subspace}
  \begin{itemize}
    ...
    \item[$\diamond$] $L(B)$ is the intersection of all subspaces of $\R^{n\times1}$ that contain $B$
      $\longrightarrow$ $L(B)$ is the ``smallest'' subspace that contains $B$
  \end{itemize}
\end{frame}
...
\begin{frame}{A Review of Linear Algebra: Subspace}
  ...
  \begin{itemize}
    \item Definition:
      ...
      $B\subseteq\R^{n\times1}$, $B$ is a linearly dependent set of vectors
      \begin{equation*}
        \Longleftrightarrow
      \end{equation*}
...
\begin{frame}{A Review of Linear Algebra: Subspace}
  \small
  \begin{itemize}
    \item Theorem 1: Every subspace, $S$ of $\R^{n\times1}$, has a basis. Any two bases of a subspace must contain the same number of elements (Dimension)
...
\begin{frame}{A Review of Linear Algebra: Subspace}
  \begin{itemize}
    \item Column space: The space spanned by the columns of $A$, which is a subspace of $\R^{m\times1}$
    \item Row space: The space spanned by the rows of $A$, which is a subspace of $\R^{n\times1}$
    \item Rank: The dimension of the column (row) space of $A$ is called the column (row) rank of $A$:
      \begin{equation*}
...
\begin{frame}{A Review of Linear Algebra: Matrix Inverse}
  \begin{itemize}
    ...
    \item $AB=BA=I_n$:
      ...
      $A$ is nonsingular, $B=A^{-1}$ is the inverse of $A$
...
\begin{frame}{A Review of Linear Algebra: Gaussian Elimination}
  \small
  \begin{itemize}
    \item Gaussian elimination: Applying forward elimination and back substitution together to solve $A\vect{x}=\vect{b}$
      \begin{itemize}
        \item Upper triangular matrix: $U$, all zeros below the main diagonal
        \item Forward elimination: Using elementary row operations on $[A\mid\vect{b}]$ to obtain $[U\mid\bar{\vect{b}}]$
        \item Back substitution: Substituting $x_m,x_{m-1},\ldots,x_{m-i}$ into the $(m-i-1)$-th equation and solving for $x_{m-i-1}$
      \end{itemize}
...
\begin{frame}{A Review of Linear Algebra: LU-decomposition}
  \begin{itemize}
    \item Row echelon form: Upper triangular (for square matrix)
      \begin{itemize}
        \item Pivot: The leftmost nonzero entry of a row
        \item Pivoting: Elementary row operations to find pivot
        \item Pivot matrix: Matrix that performs a row interchange; it is a permutation matrix obtained by applying the same interchange to $I_m$
      \end{itemize}
...
\begin{frame}{A Review of Linear Algebra: LU-decomposition}
  ...
  \begin{itemize}
    \item Apply Gaussian elimination to $A\vect{x}=\vect{b}$ for row echelon form:
      ...
      \end{equation*}
    \item Collecting the row permutations into a permutation matrix $P$ and the elimination operations into a lower-triangular matrix $L$ gives
      \begin{equation*}
        PA=LU.
      \end{equation*}
    \item Hence
